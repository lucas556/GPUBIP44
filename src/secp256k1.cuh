...
// ===================== 常量 & 全局指针 =====================
// 注意：此头文件假定只在一个 .cu 中被 include，避免多重定义问题。

#ifdef __CUDA_ARCH__

__device__ __constant__ uint64_t P_CONST[4] = {
...
__device__ __constant__ uint64_t N_CONST[4] = {
...
__device__ __constant__ uint64_t GX_CONST[4] = {
...
__device__ __constant__ uint64_t GY_CONST[4] = {
...
__device__ __constant__ uint64_t ZERO_MONT[4] = {
    0x0ULL, 0x0ULL, 0x0ULL, 0x0ULL
};

__device__ __constant__ uint64_t ONE_MONT[4] = {
...
__device__ __constant__ uint64_t SEVEN_MONT[4] = {
    0x700001AB7ULL,
    0x0ULL,
    0x0ULL,
    0x0ULL
...
__device__ __constant__ uint64_t P_CONST_MINUS_2[4] = {
...
__device__ __constant__ uint64_t LAMBDA_N[4] = {
...
__device__ __constant__ uint64_t BETA_P[4] = {
    0x7AE96A2B657C0710ULL,
    0x6E64479EAC3434E9ULL,
    0x9CF0497512F58995ULL,
    0xB315ECECBB640683ULL
};

__device__ __constant__ uint64_t MINUS_B1[4] = {
    0x6F547FA90ABFE4C3ULL,
    0xE4437ED6010E8828ULL,
    0x0000000000000000ULL,
    0x0000000000000000ULL
};

__device__ __constant__ uint64_t MINUS_B2[4] = {
...
__device__ __constant__ uint64_t G1[4] = {
...
__device__ __constant__ uint64_t G2[4] = {
...
// 这些是 device 全局指针（需要在 host 侧 cudaMalloc 并传下去 / 或者做成常量表）
...
constexpr uint64_t ZERO_MONT[4] = {
    0x0ULL, 0x0ULL, 0x0ULL, 0x0ULL
};

constexpr uint64_t ONE_MONT[4] = {
    0x00000001000003D1ULL,
    0x0ULL,
    0x0ULL,
    0x0ULL
...
constexpr uint64_t SEVEN_MONT[4] = {
    0x700001AB7ULL,
    0x0ULL,
    0x0ULL,
    0x0ULL
...
constexpr uint64_t P_CONST_MINUS_2[4] = {
    0xFFFFFFFEFFFFFC2DULL,
    0xFFFFFFFFFFFFFFFFULL,
    0xFFFFFFFFFFFFFFFFULL,
    0xFFFFFFFFFFFFFFFFULL
...
// host 侧也有对应指针（这里初始化为 nullptr，真正 cudaMalloc 可以在别的 .cpp 里做）
inline ECPointJacobian* preCompG   = nullptr;
inline ECPointJacobian* preCompGphi = nullptr;
inline ECPointJacobian* jacNorm    = nullptr;
inline ECPointJacobian* jacEndo    = nullptr;
...
struct uint128_t {
    ...
    __host__ __device__ uint128_t operator*(uint64_t other) const {
    ...
    __host__ __device__ uint128_t& operator*=(uint64_t other) {
        *this = *this * other;
        return *this;
    }
    ...
    __host__ __device__ uint128_t operator>>(const unsigned int n) const {
        uint128_t res;
        if (n == 0) return *this;
        else if (n < 64) {
            res.lo = (lo >> n) | (hi << (64 - n));
    ...
    __host__ __device__ uint128_t& operator>>=(const unsigned int n) {
        *this = *this >> n;
        return *this;
    }
    ...
    __host__ __device__ bool operator<(uint64_t other) const {
        return *this < uint128_t(other);
    }
...
__host__ __device__ inline void montgomeryReduceP(
    uint64_t *result,
    const uint64_t *inputHigh,
    const uint64_t *inputLow
) {
    uint64_t temp[8];
    for (int i = 0; i < 4; i++) {
        temp[i]     =  inputLow[i];
...
__host__ __device__ inline void toMontgomeryP(uint64_t *result, const uint64_t *a) {
    uint64_t aLocal[4];
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            uint128_t prod = (uint128_t)aLocal[i] * (uint128_t)R2_MOD_P[j]
                           + (uint128_t)temp[i + j] + carry;
            temp[i + j] = (uint64_t)prod;
            carry = prod >> 64;
        }
...
__host__ __device__ inline void fromMontgomeryP(uint64_t *result, const uint64_t *a) {
    uint64_t zero[4] = {0, 0, 0, 0};
    montgomeryReduceP(result, zero, a);
}

__host__ __device__ inline void modAddP(
    uint64_t *result,
    const uint64_t *a,
    const uint64_t *b
) {
    ...
    if (ge) {
        uint64_t borrow = 0;
        for (int i = 0; i < 4; ++i) {
            uint128_t sub = (uint128_t)result[i]
                          - (uint128_t)P_CONST[i] - borrow;
            result[i] = (uint64_t)sub;
            borrow = ((uint128_t)result[i] <
                     (uint128_t)P_CONST[i] + borrow) ? 1 : 0;
        }
    }
}

__host__ __device__ inline void modSubP(
    uint64_t *result,
    const uint64_t *a,
    const uint64_t *b
) {
    ...
    if (borrow) {
        uint128_t carry = 0;
        for (int i = 0; i < 4; ++i) {
            uint128_t s = (uint128_t)result[i]
                        + (uint128_t)P_CONST[i] + carry;
            result[i] = (uint64_t)s;
            carry = s >> 64;
        }
    }
...
__host__ __device__ inline void modMulMontP(
    uint64_t *result,
    const uint64_t *a,
    const uint64_t *b
) {
    ...
    for (int i = 0; i < 4; i++) {
        uint64_t carry = 0ULL;
        for (int j = 0; j < 4; j++) {
            uint128_t prod = (uint128_t)a[i] * (uint128_t)b[j]
                           + (uint128_t)temp[i + j]
                           + (uint128_t)carry;
            temp[i + j] = (uint64_t)prod;
...
__host__ __device__ inline void modSqrtMontP(uint64_t *out, const uint64_t *in) {
    modMulMontP(out, in, in);
}

__host__ __device__ inline void scalarReduceN(uint64_t *r, const uint64_t *k) {
    bool ge = false;
    for (int i = 3; i >= 0; i--) {
...
__host__ __device__ inline void modExpMontP(
    uint64_t *res,
    const uint64_t *base,
    const uint64_t *exp
) {
...
__host__ __device__ inline void sqrtModP(uint64_t y[4], const uint64_t v[4]) {
    uint64_t exp[4];

    exp[0] = 0x0FFFFFFFFFFFFFFFULL;
    exp[1] = 0xFFFFFFFFFFFFFFFFULL;
    exp[2] = 0xFFFFFFFFFFFFFFFFULL;
    exp[3] = 0x3FFFFFFF0000000CULL;
...
__host__ __device__ inline void jacobianInit(ECPointJacobian *point) {
    for (int i = 0; i < 4; i++) {
        point->X[i] = 0;
        point->Y[i] = 0;
...
__host__ __device__ inline void jacobianSetInfinity(ECPointJacobian *point) {
    for (int i = 0; i < 4; i++) {
...
__host__ __device__ inline int jacobianIsInfinity(const ECPointJacobian *point) {
    uint64_t z_zero = 0;
    for (int i = 0; i < 4; i++) {
        z_zero |= point->Z[i];
    }
...
__host__ __device__ inline void jacobianToAffine(
    ECPoint *aff,
    const ECPointJacobian *jac
) {
...
__host__ __device__ inline void jacobianDouble(
    ECPointJacobian *result,
    const ECPointJacobian *point
) {
...
__host__ __device__ inline void jacobianAdd(
    ECPointJacobian *result,
    const ECPointJacobian *P,
    const ECPointJacobian *Q
) {
...
__host__ __device__ inline void endomorphismMap(
    ECPointJacobian *R,
    const ECPointJacobian *P
) {
...
__host__ __device__ inline void initPrecompG(int windowSize) {
...
__host__ __device__ inline void jacobianScalarMult(
    ECPointJacobian *result,
...
__host__ __device__ inline void jacobianScalarMultPhi(
    ECPointJacobian *result,
    const uint64_t *scalar,
    int nBits,
    int windowSize
...
__host__ __device__ inline void scalarMulShiftVar(
    uint64_t r[4],
    const uint64_t a[4],
    const uint64_t b[4],
    int shift
...
__host__ __device__ inline void scalarMul(
    uint64_t r[4],
    const uint64_t a[4],
    const uint64_t b[4]
) {
...
__host__ __device__ inline void scalarAdd(
    uint64_t r[4],
    const uint64_t a[4],
    const uint64_t b[4]
) {
...
__host__ __device__ inline void scalarSub(
    uint64_t r[4],
    const uint64_t a[4],
    const uint64_t b[4]
) {
...
__host__ __device__ inline int scalarIsZero(const uint64_t a[4]) {
    uint64_t acc = 0;
    for (int i = 0; i < 4; i++) {
        acc |= a[i];
    }
...
__host__ __device__ inline void scalarNeg(uint64_t r[4], const uint64_t a[4]) {
    if (scalarIsZero(a)) {
        r[0] = r[1] = r[2] = r[3] = 0;
        return;
    }
...
__host__ __device__ inline void scalarSplitLambda(
    uint64_t r1[4],
    uint64_t r2[4],
    const uint64_t k[4]
) {
...
__host__ __device__ inline void jacobianScalarMultGlv(
    ECPointJacobian *R,
    const uint64_t k[4],
    int nBits,
    int windowSize
...
__host__ __device__ inline void pointInitJacobian(ECPointJacobian *P) {
    for (int i = 0; i < 4; i++) {
        P->X[i] = 0;
        P->Y[i] = 0;
        P->Z[i] = 0;
...
__host__ __device__ inline void pointAddJacobian(
    ECPointJacobian *R,
    const ECPointJacobian *P,
    const ECPointJacobian *Q
) {
...
__host__ __device__ inline void pointDoubleJacobian(
    ECPointJacobian *R,
    const ECPointJacobian *P
) {
    jacobianDouble(R, P);
...
__host__ __device__ inline void scalarMultJacobian(
    ECPointJacobian *R,
    const uint64_t *k,
    int nBits,
    int windowSize
) {
    jacobianScalarMultGlv(R, k, nBits, windowSize);
}

__host__ __device__ inline void getCompressedPublicKey(
    unsigned char *out,
...
// 原版：生成压缩公钥（33B）
__host__ __device__ inline void generatePublicKey(
    unsigned char *out,
    const uint64_t *PRIV_KEY,
    int nBits,
    int windowSize
) {
    ECPoint pub;
...
// ---- 追加到 secp256.cuh 末尾：字节↔uint64 转换 + GPU 公钥计算 ----

// 32B big-endian → 4×uint64（小端 limb），用于 scalar 运算
__host__ __device__ __forceinline__
void be32_to_u64le4_bytes(const unsigned char in[32], uint64_t out[4]) {
    // in[0] 是最高字节
    for (int w = 0; w < 4; ++w) {
        uint64_t v = 0;
        for (int b = 0; b < 8; ++b) {
            v = (v << 8) | (uint64_t)in[w * 8 + b];
        }
        // 把最高的 64bit 放到 out[3]，保持 N_CONST 的“小端 limb”布局
        out[3 - w] = v;
    }
}

// 4×uint64（小端 limb） → 32B big-endian
__host__ __device__ __forceinline__
void u64le4_to_be32_bytes(const uint64_t in[4], unsigned char out[32]) {
    for (int w = 0; w < 4; ++w) {
        uint64_t v = in[3 - w]; // in[3] 是最高 limb
        for (int b = 7; b >= 0; --b) {
            out[w * 8 + b] = (unsigned char)(v & 0xFFu);
            v >>= 8;
        }
    }
}

// 固定基点 G 的简单 double-and-add 标量乘（不用预计算表）
__host__ __device__
void scalarMultBaseSimple(ECPointJacobian *R, const uint64_t k[4]) {
    // 把 G 转成 Montgomery，并放到 Jacobian
    ...
    // 256 bit 标量，从最高位扫到最低位
    for (int word = 3; word >= 0; --word) {
        for (int bit = 63; bit >= 0; --bit) {
            if (!jacobianIsInfinity(R)) {
                jacobianDouble(R, R);
            }
            uint64_t bitv = (k[word] >> bit) & 1ULL;
            if (bitv) {
                if (jacobianIsInfinity(R)) {
                    *R = Q;
                } else {
...
// 从 32B 大端私钥计算压缩公钥（33B），完全在 GPU 上可用
__device__
void gpu_secp256k1_compressed_pub_from_be(const unsigned char priv_be[32],
                                          unsigned char out33[33]) {
    // 1) big-endian bytes → 小端 limb
    uint64_t k[4];
    be32_to_u64le4_bytes(priv_be, k);

    // 2) mod n 规约，避免越界
    scalarReduceN(k, k);
    if (scalarIsZero(k)) {
        // 非法标量，正常 BIP32 流程不会出现；这里直接返回全 0
    ...
    // 3) 标量乘：R = k * G
    ECPointJacobian R;
    scalarMultBaseSimple(&R, k);

    // 4) Jacobian → Affine → 压缩公钥 33B
    ECPoint A;
    jacobianToAffine(&A, &R);
    getCompressedPublicKey(out33, &A);
}
